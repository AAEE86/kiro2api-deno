...
export class ConversationIDManager {
  ...
  generateConversationID(clientIP: string, userAgent: string, customConvID?: string): string {
    ...
    const timeWindow = new Date().toISOString().slice(0, 13).replace(/[-:T]/g, "");
    ...
    const conversationID = `conv-${Array.from(hash.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join('')}`;
...
export function generateStableConversationID(clientIP: string, userAgent: string, customConvID?: string): string {
  return globalConversationIDManager.generateConversationID(clientIP, userAgent, customConvID);
}

export function generateStableAgentContinuationID(clientIP?: string, userAgent?: string, customAgentID?: string): string {
